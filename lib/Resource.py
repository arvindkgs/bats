...
class Resource(object):
    ...
    def getProperties(self, dynamicMap):
        ...
        if self.type == Type.JSON:
            properties = self.getPropertiesJSON(extrapolated_properties, files)
        elif self.type == Type.PROPERTY:
            properties = self.getPropertiesProperty(extrapolated_properties, files)
        elif self.type == Type.XML:
            properties = self.getPropertiesXML(extrapolated_properties, files)
        else:
    ...
    def getPropertiesJSON(self, extrapolated_properties, files):
        properties = []
        for fileobj in files:
            try:
                ...
                comparelog.print_error(msg="Invalid json '" + self.file + "'", args={'fnName': self.testName,
                                                                                     'type': comparelog.FILE_NOT_FOUND,
                                                                                     'source': self.file,
                                                                                     'checkName': self.checkName})
        return properties

    def getPropertiesProperty(self, extrapolated_properties, files):
        properties = []
        for fileobj in files:
            for property in extrapolated_properties:
    ...
    def getPropertiesXML(self, extrapolated_properties, files):
        properties = []
                    ...
                    if topelement is not None and len(topelement) > 0:
                        for property in extrapolated_properties:
                            try:
                                ...
                                comparelog.print_error(
                                    msg="XPath error for property: " + property + " from xml file: " + self.file,
                                    args={'fnName': self.testName, 'type': comparelog.MISSING_PROPERTY,
                                          'source': fileobj, 'checkName': self.checkName})
        return properties
